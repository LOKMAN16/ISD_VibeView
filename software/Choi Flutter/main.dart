...
class _MyHomePageState extends State<MyHomePage> {
  ...
  SpeechContext sc = SpeechContext([
    "bottle",
    "mouse",
    "cell phone",
    "laptop",
    ...
    if (text != '') {
      ...
      for (var i = 0; i < recognitions!.length; i++) {
        if (recognitions[i]['detectedClass'] == text.trim().toLowerCase()) {
          ...
          ///Update dataToArduino[1] and [2] according to object's position (left-right)
          if (angle < 7) {
            setState(() {
              moveLR = "(not visible)";
              dataToArduino[0] = 0;
          ...
          ///Update dataToArduino[2] according to object's position (up-down) WITHOUT GYRO v1
          if (height < 9) {
            setState(() {
              moveUD = "up";
              dataToArduino[2] = 0;
          ...
          ///Update dataToArduino[2] according to object's distance and position (up-down) WITH GYRO
          /*
          if (height < 30) {
            if (gyro[1] < 45) {
              moveUD = "forward";
          ...
          ///Update dataToArduino[2] according to object's distance and position (up-down) WITHOUT GYRO v2
          /*
          if (height < 30) {
            moveUD = "up";
            if (size < 10) {
          ...
          } else if (angle >= 42 && angle < 50 && height >= 30 && height < 60) {
            setState(() {
              dataToArduino[5] = 5;
            });
          } else {
      ...
      if (count == 0 && count2 == 0) {
        ...
        moveLR = "(not visible)";
        moveUD = "";
        Timer.periodic(const Duration(milliseconds: 1000), (dingTimer) {
          assetsAudioPlayer.open(
            Audio("assets/ding.mp3"),
          );
          if (angle != 0 || playDing == false) {
            dingTimer.cancel();
          }
        });
      }
  ...
  Widget gyroData(BuildContext context) {
    return Row(mainAxisAlignment: MainAxisAlignment.center, children: [
      Container(
          height: 60,
          width: 400,
  ...
  Widget sentData(BuildContext context) {
    return Row(mainAxisAlignment: MainAxisAlignment.center, children: [
      Container(
          height: 60,
          width: 400,
          ...
          child: ElevatedButton(
            ...
            child:
                Column(mainAxisAlignment: MainAxisAlignment.center, children: [
              Text(
                "Send data ${dataToArduino[0]} ${dataToArduino[1]} ${dataToArduino[2]} ${dataToArduino[3]} ${dataToArduino[4]} ${dataToArduino[5]}",
                style: const TextStyle(fontSize: 19, color: Colors.black),
              )
            ]),
          )),
